...
/**
 * صفحة فحص النظام وتنظيم الأكواد
 * تعرض نتائج فحص الترابط بين الجداول وسلامة البيانات
 */
const SystemCheckerPage = () => {
  ...
  if (!user || !(user.isAdmin || user.isSuperAdmin || user.role === 'admin' || user.role === 'superadmin' || 
      (user.permissions && (user.permissions.includes('all') || user.permissions.includes('manage_system'))))) {
    return (
  ...
  return (
    <Box p={5}>
      <Flex justify="space-between" align="center" mb={6}>
        <Box>
          <BackButton />
          <Heading size="lg">فحص النظام وتنظيم الأكواد</Heading>
          <Text mt={2} color="gray.600">
            يمكنك من خلال هذه الصفحة فحص ترابط الجداول وسلامة البيانات في النظام
          </Text>
        </Box>
      </Flex>
